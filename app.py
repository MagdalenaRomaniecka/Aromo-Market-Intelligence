...
@st.cache_data
def load_data():
    # We load the RAW file directly to avoid "File Not Found" errors on Hugging Face
    ...
    try:
        ...
        # 1. Clean 'Year': Convert to numeric, drop errors/NaNs, convert to integer
        df['year'] = pd.to_numeric(df['year'], errors='coerce')
        df = df.dropna(subset=['year'])
        df = df[df['year'] > 0] # Remove 0 years
        ...
        st.error(f"⚠️ CRITICAL ERROR: File '{file_path}' not found. Please ensure 'aromo_english.csv' is uploaded to Files.")
        return pd.DataFrame()
...
with tab3:
    ...
    # --- SIMULATION FOR DEMO ---
    # In a real scenario, this would load the embeddings model
    
    st.markdown("### ⚡ Live Analysis: Top Competitors by Scent Profile")
    st.write(f"Analyzing vector space neighbors for **{selected_brand}**...")
    
    # Dummy data for visualization purposes (Placeholder)
